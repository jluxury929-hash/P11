...
console.log("-----------------------------------------");
console.log("🟢 [BOOT] 25 ETH TITAN INITIALIZING...");
...
const CONFIG = {
    ...
    // ⚙️ STRATEGY SETTINGS
    LOAN_AMOUNT: ethers.parseEther("25"), // Fixed 25 ETH Strike
    GAS_LIMIT: 900000n, 
    ...
    try {
        ...
        wsProvider.on("block", async (blockNum) => {
            try {
                ...
                // (Only fire on blocks with actual value transfers to save API credits)
...
async function executeTitanStrike(provider, signer, titanContract, oracle) {
    try {
        ...
        const loanAmount = CONFIG.LOAN_AMOUNT; // Fixed 25 ETH
